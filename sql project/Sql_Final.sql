...
-- CALENDAR TABLE SALES, QUANTITY and TOTAL ORDERS
-- 
SELECT
...
-- Sales over Weekdays and Weekends
...
SELECT AVG(total_sales) AS average_sales -- TAKES THIS AVG SALES into internal query
FROM (
    SELECT 
        SUM(unit_price * transaction_qty) AS total_sales
    FROM 
	...
	WHERE 
        MONTH(transaction_date) = 5  -- Filter for May
    GROUP BY 
        transaction_date -- for each and every day 
) AS internal_query;
...
/*
Query Explanation:
This inner subquery calculates the total sales (unit_price * transaction_qty) for each date in May.
 It filters the data to include only transactions that occurred in May 
 by using the MONTH() function to extract the month from the transaction_date column 
 and filtering for May (month number 5).
The GROUP BY clause groups the data by transaction_date,
 ensuring that the total sales are aggregated for each individual date in May.
 
The outer query calculates the average of the total sales over all dates in May.
 It references the result of the inner subquery as a derived table named internal_query.
